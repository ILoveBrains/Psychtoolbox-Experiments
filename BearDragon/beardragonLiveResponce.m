function beardragonLiveResponce(subnum,famnum,EEGisOn,debugMode)
%% Modified version of ExpCorInc to include a half point for 
%(half points for not following dragon command but using strategy)
%% ExpCorInc Code start
...
rr = KbName('r');
tt = KbName('t');
RestrictKeysForKbCheck([49 48])

%Loop for period of time given by maxT input

while moveOn == 0
 %% CURRENTLY CONFIGURED FOR ERIKS WORK IMAC (KBDEVICE 6)   
